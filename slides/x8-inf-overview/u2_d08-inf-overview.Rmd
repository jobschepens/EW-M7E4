...
.small[
.question[
`r emo::ji("busts_in_silhouette")` Describe the simulation process for estimating the parameter
...
- Specify whether the simulation type would be bootstrap, simulate, or permute.
  - bootstrap: take x samples from the sample with replication 
  - simulate: draw x samples randomly from a theoretical distribution, e.g. binomial
  - permute: shuffle the sample and reassign (re-assign labels)
